...
import bd.gov.forms.domain.Track;
...
public class FormDaoImpl implements FormDao {
        ...
        jdbcTemplate.execute(sql,
                new AbstractLobCreatingPreparedStatementCallback(lobHandler) {
                    ...
                    protected void setValues(PreparedStatement ps, LobCreator lobCreator) throws SQLException {
                        ...
                        ps.setString(i++, form.getMinistry());
                        ...
                        if (formHasLogo(form)) {
                            lobCreator.setBlobAsBinaryStream(ps, i++, new ByteArrayInputStream(form.getLogoTemplate()),
                                    form.getLogoTemplate().length);
                            ps.setString(i, form.getLogoFileName());
                        }
                    }
                }
        );*/
    ...
    private boolean formHasTemplate(Form form) {
     ...
     private boolean formHasLogo(Form form) {
        return form.getLogoTemplate() != null && form.getLogoTemplate().length > 0;
    }
    ...
    public Form getForm(String formId) {
        ...
        return (Form) sessionFactory.getCurrentSession().createCriteria(Form.class).add(Restrictions.eq("formId", formId)).list().get(0);
        /*return (Form) jdbcTemplate.queryForObject(
                ...
                new RowMapper() {
                    ...
                    public Object mapRow(ResultSet resultSet, int rowNum) throws SQLException {
                        ...
                        form.setMinistry((resultSet.getString("ministry")));
                        form.setTemplateFileName(resultSet.getString("template_file_name"));
                        form.setLogoFileName(resultSet.getString("logo_file_name"));
    ...
    public void updateForm(final Form form) {
        ...
        Form fm = (Form)sessionFactory.getCurrentSession().createCriteria(Form.class).
                add(Restrictions.eq("formId", form.getFormId())).list().get(0);
    ...
    public byte[] getTemplateContent(String formId) {
        ...
        Form form = (Form) sessionFactory.getCurrentSession().createCriteria(Form.class)
                .setProjection(Property.forName("pdfTemplate"))
                .add(Property.forName("formId").eq(formId));
    ...
    public Form getFormWithFields(String formId) {
        ...
        if (form != null) {
            ...
            /*List fields = (List<Field>) sessionFactory.getCurrentSession().createCriteria(Field.class)
                    .add(Restrictions.eq("formId", form.getId())).list();*/
            List fields = (List<Field>) sessionFactory.getCurrentSession().createCriteria(Field.class)
                    .add(Property.forName("formId").eq(form.getId())).addOrder(Order.asc("fieldOrder")).list();
    ...
    public Field getField(String fieldId) {
        ...
        return (Field) sessionFactory.getCurrentSession().createCriteria(Field.class)
                .add(Restrictions.eq("fieldId", fieldId)).list().get(0);
    }
    ...
    public List getFormList(int page, String ministryName) {
        /*String sql = "select * from form where ministry = '"+ministryName+"'";
        ...
        return sessionFactory.getCurrentSession().createCriteria(Form.class)
                .add(Property.forName("ministry").eq(ministryName)).list();
    }
    ...
    private void populateFormFromResultSet(ResultSet rs, Form form) throws SQLException {
        ...
        form.setParent(rs.getString("parent"));
    }
    
    public List getPublicForms() {
        String sql = "select * from form where status=2";

        //return (List<Form>) sessionFactory.getCurrentSession().createCriteria(Form.class).add(Restrictions.eq("status", 2)).list();
        return (List<Form>) sessionFactory.getCurrentSession().createSQLQuery(sql).addEntity(Form.class).list();
    }
    ...
    public void removeLogoTemplate(String formId) {
        jdbcTemplate.update("UPDATE form set  logo_file = null, logo_file_name = null WHERE form_id = ?",
                formId);
    }
    ...
    public List getParents(String formId) {
        //String sql = "select parent from form where form_id = '"+formId+"'";
        /*List parents = sessionFactory.getCurrentSession().createCriteria(Form.class)
                .add(null)*/
        String a = (String) sessionFactory.getCurrentSession().createCriteria(Form.class)
        ...
        if(a == null) {
            ...
            while( a != null) {
               ...
               // a = form.getFormId();
                ...
                //String sq = "select parent from form where form_id = '"+a+"'";
                a = (String) sessionFactory.getCurrentSession().createCriteria(Form.class)
                            .setProjection(Property.forName("parent"))
                            .add(Property.forName("formId").eq(a)).list().get(0);
                i++;               
            }
        }     
        return list;
    ...
    public Object getFirstMilestoneId(int workflowId){
        return sessionFactory.getCurrentSession().createCriteria(WorkFlowDetail.class)
                .add(Property.forName("workflowId").eq(workflowId))
                .setProjection(Property.forName("milestoneId"))
                .addOrder(Property.forName("id").asc()).list().get(0);
    }
    
    public void updateTrackMilestone(int milestoneId, String trackId){
        jdbcTemplate.update("UPDATE track set current_milestone_id=? where track_id=?", milestoneId, trackId);
    }
